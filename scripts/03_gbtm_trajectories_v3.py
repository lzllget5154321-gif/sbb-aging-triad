#!/usr/bin/env python
"""GBTM v3 — Mixed Strategy (方案 D)

BAI: ≥3 waves full intercept+slope GBTM; 2 waves intercept-only clustering
BoAI: ≥3 waves intercept-only; 2 waves intercept-only (slope unreliable per E3 validation)
SAI: unchanged from v2 (≥3 waves, feature-based)

Key changes from v2:
1. BAI ≥3 waves only for trajectory clustering (not ≥2)
2. BoAI drops slope dimension entirely (77% unstable per E3)
3. 2-wave individuals get intercept-only cross-sectional grouping
4. Groups labeled to distinguish trajectory vs cross-sectional
"""
